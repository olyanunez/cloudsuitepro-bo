...
import { Role, Permission, RoleUpdateInput } from '@/lib/types/role';
...
export default function EditRolePage() {
  ...
  const handlePermissionChange = (e: React.ChangeEvent<HTMLInputElement>, screenId: number) => {
    ...
    // Si se está desmarcando un permiso, verificamos si está en los permisos iniciales
    if (!checked && screenPermissionId > 0) {
      console.log(`Permiso desmarcado con screenPermissionId=${screenPermissionId}`);
      console.log(`Está en permisos iniciales: ${initialPermissions.includes(screenPermissionId)}`);
    }
    ...
    setFormData(prev => {
      ...
      if (checked) {
        ...
        const removedPair = updatedPairs.find(p => p.screenId === screenId && p.permissionId === permissionId);
        if (removedPair && removedPair.screenPermissionId > 0) {
          console.log(`Removiendo par con screenPermissionId=${removedPair.screenPermissionId}`);
        }
      ...
      console.log('Comparando con permisos iniciales:', initialPermissions);
  ...
  // Almacena los permisos iniciales cuando se carga el rol
  ...
  // Actualizar initialPermissions solo una vez cuando se carga el rol
  useEffect(() => {
    if (
      role &&
      Array.isArray(role.screensWithPermissions) &&
      !initialPermissionsLoaded
    ) {
      const initialPermIds: number[] = [];
      role.screensWithPermissions.forEach((screenWithPerm: { permissions: Permission[] }) => {
        screenWithPerm.permissions.forEach((permission: Permission & { screenPermissionId?: number }) => {
          if (permission.screenPermissionId && permission.screenPermissionId > 0) {
            initialPermIds.push(permission.screenPermissionId);
          }
        });
      });
      const validUniquePermIds = [...new Set(initialPermIds.filter(id => id > 0))];
      setInitialPermissions(validUniquePermIds);
      setInitialPermissionsLoaded(true);
      setFormData(prev => ({
        ...prev,
        screenPermissionIds: validUniquePermIds
      }));
    }
  }, [role, initialPermissionsLoaded]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentPermissionIds = screenPermissionPairs
        .filter(p => p.screenPermissionId && p.screenPermissionId > 0)
        .map(p => p.screenPermissionId as number);
      // Enviar los arrays inicial y actual para que el servicio calcule los cambios
      const dataToSend: RoleUpdateInput & {
        initialScreenPermissionIds: number[];
        selectedScreenPermissionIds: number[];
      } = {
        name: formData.name || '',
        code: formData.code || '',
